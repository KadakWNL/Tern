...
def get_chapter_questions(blueprint_path: str) -> Dict[str, List[int]]:
    ...
    table=table[2::]
    # Convert table format to match the original camelot output
    blueprint_data = {0: [], 1: []}
    for row in table:
        if row[0] is not None:  # Chapter name column
            blueprint_data[0].append(row[0])
        if row[1] is not None:  # Multiple choice question column
            blueprint_data[1].append(row[1])
    
    chaper_names = list(blueprint_data[0])
    chaper_names.pop()  # Remove the last item (Total)
    multiple_choice_questions = list(blueprint_data[1])
    multiple_choice_questions.pop()  # Remove the last item
    
    # Extract first character of each question number
