...
student_analysis=pd.read_excel(r"Tern\Resources\student_analysis.xls",skiprows=8)
...
expanded_scorelist=pd.read_excel(r'Tern\Resources\expanded_scorelist.xlsx')
...
for p in range(0,len(expanded_scorelist)):
    ...
    attempt_wrong=[]
    left=[]  
    attempt_correct=list(map(making_sure_empty_cells_are_noticed,str(data2["MATHEMATICS R IDS"]).split(',') ))
    print(str(data2["MATHEMATICS R IDS"]))
    attempt_wrong=list(map(making_sure_empty_cells_are_noticed,str(data2["MATHEMATICS W IDS"]).split(',') ))
    left=list(map(making_sure_empty_cells_are_noticed,str(data2["MATHEMATICS L IDS"]).split(',') ))
    ...
    for chapter_name,questions in chapterwise_question_numbers.items():
        # print(attempt_correct, attempt_wrong)
